...
impl<'a> Parser<'a> {
    ...
    pub fn check_strict_keywords(&mut self) {
        if self.token.is_strict_keyword() {
            ...
            self.span_err(
                span,
                &format!("expected identifier, found keyword `{}`", token_str),
            );
        }
    }
    ...
    /// f must consume tokens until reaching the next separator or
    /// closing bracket.
    pub fn parse_seq_to_before_end<T, F>(&mut self, ket: &token::Token, sep: SeqSep, f: F) -> Vec<T>
    where
        F: FnMut(&mut Parser<'a>) -> PResult<'a, T>,
    {
    ...
    fn parse_seq_to_before_tokens<T, F, Fe>(
        &mut self,
        kets: &[&token::Token],
        sep: SeqSep,
        mut f: F,
    ...
    pub fn is_named_argument(&mut self) -> bool {
        ...
        if offset == 0 {
            is_ident_or_underscore(&self.token) && self.look_ahead(1, |t| *t == token::Colon)
        } else {
    ...
    /// `<T as U>::a`
    /// `<T as U>::F::a::<S>`
    pub fn parse_qualified_path(&mut self, mode: PathStyle) -> PResult<'a, (QSelf, ast::Path)> {
        let span = self.prev_span;
        ...
        let segments = match mode {
            PathStyle::Type => self.parse_path_segments_without_colons()?,
            PathStyle::Expr => self.parse_path_segments_with_colons()?,
            PathStyle::Mod => self.parse_path_segments_without_types()?,
        };
    ...
    pub fn parse_path_segments_without_colons(&mut self) -> PResult<'a, Vec<PathSegment>> {
        ...
        loop {
            ...
            if self.check(&token::ModSep) && self.look_ahead(1, |t| *t == token::Lt) {
                ...
                let mut err = self
                    .diagnostic()
                    .struct_span_err(prev_span, "unexpected token: `::`");
                err.help("use `<...>` instead of `::<...>` if you meant to specify type arguments");
                err.emit();
            }
            ...
            let parameters = if self.eat_lt() {
                ...
                Some(P(ast::PathParameters::Parenthesized(
    ...
    /// Examples:
    /// - `a::b::c`
    pub fn parse_path_segments_without_types(&mut self) -> PResult<'a, Vec<PathSegment>> {
        let mut segments = Vec::new();
        loop {
            // First, parse an identifier.
            let ident_span = self.span;
    ...
    pub fn parse_prefix_expr(
        ...
        let (hi, ex) = match self.token {
            ...
            token::Ident(..) if self.token.is_keyword(keywords::In) => {
                self.bump();
                let place =
                    self.parse_expr_res(Restrictions::RESTRICTION_NO_STRUCT_LITERAL, None)?;
    ...
    pub fn parse_assoc_expr_with(&mut self, min_prec: usize, lhs: LhsExpr) -> PResult<'a, P<Expr>> {
        ...
        while let Some(op) = AssocOp::from_token(&self.token) {
            ...
            let rhs = match op.fixity() {
                Fixity::Right => self
                    .with_res(restrictions - Restrictions::RESTRICTION_STMT_EXPR, |this| {
    ...
    pub fn parse_arm(&mut self) -> PResult<'a, Arm> {
        ...
        let require_comma =
            !classify::expr_is_simple_block(&expr) && self.token != token::CloseDelim(token::Brace);
                    ...
                    && self.parse_as_ident()
                {
                    ...
                    match self.token {
                        ...
                        token::DotDotDot | token::DotDot => {
                            let end_kind = match self.token {
                                ...
                                _ => panic!(
                                    "can only parse `..` or `...` for ranges (checked above)"
                                ),
                            };
                            // Parse range
                            let span = lo.to(self.prev_span);
                    ...
                    match self.parse_pat_literal_maybe_minus() {
                        ...
                        Err(mut err) => {
                            self.cancel(&mut err);
                            let msg =
                                format!("expected pattern, found {}", self.this_token_descr());
                            return Err(self.fatal(&msg));
                        }
                    }
                }
            ...
            && !self.is_union_item()
        {
            ...
            match item {
                ...
                None => {
                    ...
                    let e = self
                        .parse_expr_res(Restrictions::RESTRICTION_STMT_EXPR, Some(attrs.into()))?;
                    Stmt {
                        id: ast::DUMMY_NODE_ID,
                        span: lo.to(e.span),
    ...
    fn expr_is_complete(&mut self, e: &Expr) -> bool {
        self.restrictions
            .contains(Restrictions::RESTRICTION_STMT_EXPR)
    ...
    fn parse_item_trait(&mut self, unsafety: Unsafety) -> PResult<'a, ItemInfo> {
        ...
        Ok((
            ident,
            ItemKind::Trait(unsafety, tps, bounds, trait_items),
            None,
        ))
    }
    ...
    fn parse_item_impl(
        ...
        if opt_trait.is_some() && self.eat(&token::DotDot) {
            ...
            Ok((
                keywords::Invalid.ident(),
                ItemKind::DefaultImpl(unsafety, opt_trait.unwrap()),
                None,
            ))
        } else {
...
`pub(in path::to::module)`: visible only on the specified path"##;
    ...
    fn parse_item_(
            ...
            && self.look_ahead(1, |t| t.is_keyword(keywords::Trait))
        {
            ...
            let (ident, item_, extra_attrs) = self.parse_item_trait(ast::Unsafety::Unsafe)?;
            let prev_span = self.prev_span;
            let item = self.mk_item(
                lo.to(prev_span),
                ident,
        ...
        if self.eat_keyword(keywords::Trait) {
            // TRAIT ITEM
            let (ident, item_, extra_attrs) = self.parse_item_trait(ast::Unsafety::Normal)?;
            let prev_span = self.prev_span;
            let item = self.mk_item(
                lo.to(prev_span),
                ident,
