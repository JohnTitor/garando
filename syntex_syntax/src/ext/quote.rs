...
pub mod rt {
    ...
    impl ToTokens for ast::Stmt {
        fn to_tokens(&self, _cx: &ExtCtxt) -> Vec<TokenTree> {
            ...
            let mut tts = vec![TokenTree::Token(
                self.span,
                token::Interpolated(Rc::new(nt)),
            )];
    ...
    macro_rules! impl_to_tokens_int {
        (signed, $t:ty, $tag:expr) => {
            impl ToTokens for $t {
                fn to_tokens(&self, cx: &ExtCtxt) -> Vec<TokenTree> {
                    let val = if *self < 0 { -self } else { *self };
                    let lit = ast::LitKind::Int(
                        i128::from(val as i64) as u128,
                        ast::LitIntType::Signed($tag),
                    );
                    let lit = P(ast::Expr {
                        id: ast::DUMMY_NODE_ID,
