...
use crate::ast;
use crate::ast::{AttrId, Attribute, Ident, Name};
use crate::ast::{Expr, ExprKind, Item, Lit, LitKind, Local, Stmt, StmtKind};
    ...
    match cfg.node {
        ast::MetaItemKind::List(ref mis) => {
            ...
            match &*cfg.name.as_str() {
                "any" => mis
                    .iter()
                    .any(|mi| cfg_matches(mi.meta_item().unwrap(), sess, features)),
                "all" => mis
                    .iter()
                    .all(|mi| cfg_matches(mi.meta_item().unwrap(), sess, features)),
                "not" => {
                    if mis.len() != 1 {
                        span_err!(
                            sess.span_diagnostic,
        ...
        I: Iterator<Item = TokenTree>,
    {
        if let Some(TokenTree::Token(span, token)) = tokens.peek().cloned() {
            if let Some(node) = LitKind::from_token(token) {
                tokens.next();
                return Some(NestedMetaItemKind::Literal(Spanned {
                    node: node,
                    span: span,
                }));
            }
...
impl LitKind {
    fn token(&self) -> Token {
        ...
        match *self {
            ...
            LitKind::Bool(value) => {
                Token::Ident(Ident::with_empty_ctxt(Symbol::intern(if value {
                    "true"
                } else {
                    "false"
                })))
